...
let metas = await Promise.map(asnkeys, async (key) => {
  try {
    ...
    const res = await con.get({ path: '/bookmarks/services/target/jobs/'+key+'/_meta' })
    const meta = res.data;
    meta.key = key;
    return meta;
  } catch(e) {
...
console.log('Have the metas, here are the target entries, sorted with oldest on top');

metas = _.sortBy(metas, m => m.modified);

_.each(metas, m => {
  const day = moment(m.modified, 'X').format('YYYY-MM-DD HH:mm:ss');
  console.log(`${m.key}: ${day}: `, JSON.stringify(m.services.target ? m.services.target.tasks : 'NO TARGET META ENTRY!!', false, '  '));
});
