...
trace(`Connected to oada, domain = ${domain}, token = ${tokenToRequestAgainstOADA}`);
...
let tests = {
  ...
  bookmarks: {
    desc: `Is bookmarks up for token ${tokenToRequestAgainstOADA.slice(0,1)}..${tokenToRequestAgainstOADA.slice(-2)}`,
    type: 'pathTest',
...
const check = async () => {
  try {
    ...
    const results = await Promise.map(testkeys, async (tk) => {
      trace(`Running test ${tk}`);
      try {
        const t = tests[tk];
        const runner = testers[t.type];
        if (!runner) return { status: 'failure', message: 'Invalid tester type' };
        return await runner({ ...(t.params), oada })
      } catch(e) {
        info(`Test ${tk} threw uncaught exception: `, e);
        return { status: 'failure', message: `Uncaught exception: ${e.toString()}` };
      }
    }, { concurrency: 1 });

    trace('Results of tests: ', results);
    status.tests = _.zipObject(testkeys, results);
    const failures = _.filter(results, r => !r.status || r.status !== 'success');
    ...
    info(`Failure: sending notification.  Failure status is: `, JSON.stringify(status,null,'  '));
    if (notifyurl) {
    ...
    error(`check: Uncaught error from main check.  Error was: ${e.toString()}`);
  }
};
...
app.get('/', async (req, res) => {
  ...
  info(`Responding to request with current global status ${status.global.status}`);
  res.json(status);
...
app.get('/trigger', async (req, res) => {
  ...
  await check();
  info(`trigger: Responding to request with current global status ${status.global.status}`);
  res.json(status);
...
app.listen(config.get('port'), () => console.log(`trellis-monitor listening on port ${config.get('port')}`));
